...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        float widthNormalization = uPillarWidth / 4.0;
  ...
  if (!webGLSupported) {
    return (
      <div
        className={`w-full h-screen relative top-0 left-0 flex items-center justify-center bg-black/10 text-gray-500 text-sm ${className}`}
        style={{ mixBlendMode }}
      >
        WebGL not supported
      </div>
  ...
  return (
    <div ref={containerRef} className={`w-full h-screen relative top-0 left-0 ${className}`} style={{ mixBlendMode }} />
  )
}

...
export default function MarketTrendsAllTable() {
  return (
    <Card className="flex flex-col w-fit h-[400px]">
      <CardHeader className="flex items-center space-y-0 border-b py-2 sm:flex-row">
        <div className="grid flex-1 text-left text-sm">
          <span>Overview</span>
        </div>
      </CardHeader>
      <CardContent className="w-fit overflow-y-auto p-0">
        <div className="border-b">
          <Table>
            <TableHeader>
              <TableRow className="h-6">
                <TableHead className="py-1 px-2 text-xs">Asset</TableHead>
                <TableHead className="py-1 px-2 text-xs">Price</TableHead>
                <TableHead className="py-1 px-2 text-xs">YTD</TableHead>
                <TableHead className="py-1 px-2 text-xs">Quarter</TableHead>
                <TableHead className="py-1 px-2 text-xs">Week</TableHead>
                <TableHead className="py-1 px-2 text-xs">Day</TableHead>
                <TableHead className="py-1 px-2 text-xs">Hourly</TableHead>
                <TableHead className="py-1 px-2 text-xs">Status</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {marketTrends.map((trend) => (
                <TableRow className="h-8" key={trend.asset}>
                  <TableCell className="py-1 px-2 text-xs">{trend.asset}</TableCell>
                  <TableCell className="py-1 px-2 text-xs">{trend.currentValue}</TableCell>
                  <TableCell className={`py-1 px-2 text-xs ${getChangeClass(trend.ytdChange)}`}>
                    {trend.ytdChange}
                  </TableCell>
                  <TableCell className={`py-1 px-2 text-xs ${getChangeClass(trend.quarterChange)}`}>
                    {trend.quarterChange}
                  </TableCell>
                  <TableCell className={`py-1 px-2 text-xs ${getChangeClass(trend.weekChange)}`}>
                    {trend.weekChange}
                  </TableCell>
                  <TableCell className={`py-1 px-2 text-xs ${getChangeClass(trend.dayChange)}`}>
                    {trend.dayChange}
                  </TableCell>
                  <TableCell className={`py-1 px-2 text-xs ${getChangeClass(trend.hourlyChange)}`}>
                    {trend.hourlyChange}
                  </TableCell>
                  <TableCell className={`py-1 px-2 text-xs ${getStatusClass(trend.status)}`}>{trend.status}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>

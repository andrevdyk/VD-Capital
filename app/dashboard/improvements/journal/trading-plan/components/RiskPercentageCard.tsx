...
export function RiskPercentageCard({ initialStrategies, onStrategiesChange }: RiskPercentageCardProps) {
    ...
    const fetchRiskStrategies = async () => {
      try {
        ...
        if (fetchedStrategies && fetchedStrategies.length > 0) {
          ...
          onStrategiesChange(predefinedStrategies)
        }
      } catch (error) {
      ...
      prevStrategies.map(strategy =>
        strategy.name === name
          ? {
              ...
              ...(field === 'selected'
                ? { [field]: Boolean(value) }
                : field.startsWith('additionalParams.')
                  ? {
                      additionalParams: {
                  ...
                  : { [field]: field === 'value' ? (typeof value === 'string' ? parseFloat(value) : value) : value }
            )
          }
          : strategy
      )
